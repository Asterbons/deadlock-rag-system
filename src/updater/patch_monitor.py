...
def download_game_files():
    ...
    for key, remote_path in WATCHED_FILES.items():
        ...
        if resp.status_code == 200:
            ...
            print(f"  WARNING: Failed to download {remote_path} ({resp.status_code})")


def check_and_update() -> bool:
    """Check GitHub for new commits and download+reindex if relevant files changed."""
    print("Checking for Deadlock game updates...")
    try:
        latest_sha = get_latest_commit_sha()
        known_sha = get_last_known_sha()

        if known_sha is None:
            print("No previous version known — downloading all files.")
            download_game_files()
            save_sha(latest_sha)
            return True

        if latest_sha == known_sha:
            print(f"No updates. Current: {latest_sha[:8]}")
            return False

        print(f"New commit detected: {known_sha[:8]} → {latest_sha[:8]}")
        changed = get_changed_files(known_sha, latest_sha)
        relevant = [f for f in changed if f in WATCHED_FILES.values()]
        print(f"Changed watched files: {relevant}")

        if should_reindex(changed):
            print("Relevant files changed — downloading and reindexing.")
            download_game_files()
            save_sha(latest_sha)
            return True
        else:
            print("No relevant files changed. Updating SHA only.")
            save_sha(latest_sha)
            return False

    except Exception as e:
        print(f"ERROR in patch monitor: {e}")
        return False
